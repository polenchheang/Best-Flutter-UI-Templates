...
class _DrawerUserControllerState extends State<DrawerUserController>
  ...
  void initState() {
    ...
    scrollController = ScrollController(initialScrollOffset: widget.drawerWidth)
      ..addListener(onHorizontalScroll);
    super.initState();
  }
  ...
  void onHorizontalScroll() {
    ...
    } else if (offset > 0 && offset < widget.drawerWidth) {
      iconAnimationController.animateTo(
          (offset * 100 / (widget.drawerWidth)) / 100,
          duration: const Duration(milliseconds: 0),
          curve: Curves.linear);
    } else if (offset <= widget.drawerWidth) {
      if (isDrawerOpen) {
        setState(() {
          isDrawerOpen = false;
          try {
  ...
  SizedBox buildDrawerView(BuildContext context) {
    return SizedBox(
